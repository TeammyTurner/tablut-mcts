...
class Node(object):
    ...
    def __init__(self, game, parent=None, move=None, remaining_moves=50, C=np.sqrt(2), heuristic=None):
        ...
        self.legal_moves = [flatten_move(
            *x) for x in self.possible_moves() if game.board.is_legal(game.turn, *x)[0]]
    ...
    def expand(self):
        ...
        while not current.game.ended and current.remaining_moves > 0:
            ...
            def what_if_board(m): return current.game.what_if(
                *deflatten_move(m)).board.board
            move_priors = [
